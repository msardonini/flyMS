...
int FlightCore::flight_core(StateData &imu_data_body) {
  ...
  u_euler_[0] = update_filter(flight_filters_->roll_inner_PID, droll_setpoint - imu_data_body.eulerRate[0]);
  ...
  u_euler_[1] = update_filter(flight_filters_->pitch_inner_PID, dpitch_setpoint - imu_data_body.eulerRate[1]);
  ...
  u_euler_[2] = update_filter(flight_filters_->yaw_PID, setpoint.euler_ref[2] - imu_data_body.euler[2]);
   ...
   *
   ************************************************************************/
  u_[0] = setpoint.throttle + u_euler_[0] - u_euler_[1] - u_euler_[2];
  u_[1] = setpoint.throttle - u_euler_[0] - u_euler_[1] + u_euler_[2];
  u_[2] = setpoint.throttle + u_euler_[0] + u_euler_[1] + u_euler_[2];
  u_[3] = setpoint.throttle - u_euler_[0] + u_euler_[1] - u_euler_[2];
   ...
   ************************************************************************/
  if (setpoint.kill_switch[0] < 0.5 && !is_debug_mode_) {
  ...
  struct ULogFlightMsg flight_msg {
    imu_data_body.timestamp_us, imu_data_body, setpoint, u_, u_euler_
  };
...
int FlightCore::ConsolePrint(const StateData &imu_data_body, const SetpointData &setpoint) {
  ...
  // u_[1], u_[2], u_[3]); spdlog::info("Aux {:2.1f} ", setpoint.Aux[0]);
  //  spdlog::info("function: {}",rc_get_dsm_ch_normalized(6));
  //  spdlog::info("num wraps {} ",control->num_wraps);
  spdlog::info(" Throt {:2.2f}, Roll_ref {:2.2f}, Pitch_ref {:2.2f}, Yaw_ref {:2.2f} ", setpoint.throttle,
               setpoint.euler_ref[0], setpoint.euler_ref[1], setpoint.euler_ref[2]);
...
int FlightCore::StartupRoutine() {
  ...
  if (config_params_["mavlink_interface"]["enable"].as<bool>()) {
    mavlink_interface_ = std::make_unique<MavlinkInterface>(config_params_["mavlink_interface"]);
