...
void DigitalFilter::zero_values() { set_to_value(0.); }

void DigitalFilter::set_to_value(double value) {
  std::for_each(signal_hist_.begin(), signal_hist_.end(), [&](auto &val) { val = value; });
  std::for_each(filter_hist_.begin(), filter_hist_.end(), [&](auto &val) { val = value; });
}
