...
std::vector<float> AttitudeController::calculate_control_loop(const std::vector<float>& setpoints,
  ...
  if (flight_mode == FlightMode::STABILIZED) {
    roll_rate_setpoint = roll_outer_pid_.update_filter(setpoints[kFLYMS_ROLL_INDEX] - imu_data_body.euler[0]);
    pitch_rate_setpoint = pitch_outer_pid_.update_filter(setpoints[kFLYMS_PITCH_INDEX] - imu_data_body.euler[1]);
  } else if (flight_mode == FlightMode::ACRO) {
    roll_rate_setpoint = setpoints[kFLYMS_ROLL_INDEX];
    pitch_rate_setpoint = setpoints[kFLYMS_PITCH_INDEX];
  } else {
    throw std::runtime_error("[FlightCore] Error! Invalid flight mode. Shutting down now");
  }

  // Roll Inner Loop
  state_control_efforts[kFLYMS_ROLL_INDEX] =
      roll_inner_pid_.update_filter(roll_rate_setpoint - imu_data_body.eulerRate[0]);
  state_control_efforts[kFLYMS_ROLL_INDEX] =
      std::clamp(state_control_efforts[kFLYMS_ROLL_INDEX], -max_control_effort_[0], max_control_effort_[0]);

  // Pitch Inner Loop
  state_control_efforts[kFLYMS_PITCH_INDEX] =
      pitch_inner_pid_.update_filter(pitch_rate_setpoint - imu_data_body.eulerRate[1]);
  state_control_efforts[kFLYMS_PITCH_INDEX] =
      std::clamp(state_control_efforts[kFLYMS_PITCH_INDEX], -max_control_effort_[1], max_control_effort_[1]);

  // Yaw Controller
  state_control_efforts[kFLYMS_YAW_INDEX] =
      yaw_pid_.update_filter(setpoints[kFLYMS_YAW_INDEX] - imu_data_body.euler[2]);
  // Apply a saturation filter
  state_control_efforts[kFLYMS_YAW_INDEX] =
      std::clamp(state_control_efforts[kFLYMS_YAW_INDEX], -max_control_effort_[2], max_control_effort_[2]);

  // Throttle Controller
  state_control_efforts[kFLYMS_THROTTLE_INDEX] = setpoints[kFLYMS_THROTTLE_INDEX];
